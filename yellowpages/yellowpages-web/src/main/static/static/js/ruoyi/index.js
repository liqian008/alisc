...
$(function() {
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .hlmy_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            $(this).show().siblings('.hlmy_iframe').hide();
                            return false;
                        }
                    });
                }
                flag = false;
                var target = $('.hlmy_iframe[data-id="' + dataUrl + '"]');
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="hlmy_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
            $('.mainContent').find('iframe.hlmy_iframe').hide().parents('.mainContent').append(str1);
    ...
    function closeOtherTabs() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.hlmy_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function refreshTab() {
    	...
    	var target = $('.hlmy_iframe[data-id="' + currentId + '"]');
        var url = target.attr('src');
        target.attr('src', url).ready();
    }
    ...
    $('#dop').on('click', function () {
        if($(this).hasClass('open')){
            ...
            $(this).find('span').text(' 打开目录');
            $(".menu-list-li").siblings().removeClass("active").children("ul.in").collapse("hide");
        }else{
            // 已经折叠
            $(this).addClass('open');
            $(this).find('span').text(' 折叠目录');
            $(".menu-list-li").not(".active").has("ul").children("ul").addClass("collapse in");
        }
    });
    ...
    $('.tabCloseAll').on('click', function() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            $('.hlmy_iframe[data-id="' + $(this).data('id') + '"]').show();
