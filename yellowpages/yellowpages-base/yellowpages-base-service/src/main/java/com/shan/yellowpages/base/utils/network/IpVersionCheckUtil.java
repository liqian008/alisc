 ...
 * @author wulinfei 
 * @date 2019年4月17日
 * Description:根据ip地址，判断是IPV4还是IPV6
 */
public class IpVersionCheckUtil {
    ...
    public static String[] checkIPs(String[] ip_array) {
        ...
        for (int i = 0; i < ip_array.length; i++) {
            if (patternIpv6.matcher(ip_array[i]).matches()) {
                str[i] = "IPv6";
            } else if (patternIpv4.matcher(ip_array[i]).matches()) {
                str[i] = "IPv4";
            } else {
                str[i] = "Neither";
            }
    ...
    public static String checkIPVersion(String ip) {
        if (patternIpv6.matcher(ip).matches()) {
            return "IPv6";
        } else if (patternIpv4.matcher(ip).matches()) {
            return "IPv4";
        } else {
            return "Neither";
        }
